...
function wp_rss_retriever_func( $atts, $content = null ){
    ...
    $output = '<div class="wp_rss_retriever">';
        $output .= '<ul>';
            if ( $maxitems == 0 ) : 
                $output .= '<li>' . _e( 'No items', 'my-text-domain' ) . '</li>';
            else : 
                // Loop through each feed item and display each item.
                foreach ( $rss_items as $item ) :
                    $output .= '<li>';
                        $output .= '<a ' . $newWindowOutput . 'href="' . esc_url( $item->get_permalink() ) . '"
                            title="' . sprintf( __( 'Posted %s', 'my-text-domain' ), $item->get_date('j F Y | g:i a') ) . '">';
                            $output .= esc_html( $item->get_title() );
                        $output .= '</a>';
                        if ( $excerpt > 0 ) {
                            $content = $item->get_content();
                            $output .= '<p>' . esc_html(implode(' ', array_slice(explode(' ', strip_tags($content)), 0, $excerpt))) . "..." . '</p>';
                        } else {
                            $output .= '<p>' . $item->get_content() . '</p>';
                        }
                        if( $read_more == 'true' ) {
